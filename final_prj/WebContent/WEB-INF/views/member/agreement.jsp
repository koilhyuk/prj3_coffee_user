...
<style type="text/css">
	...
	/* 헤더시작 */
	#header{width: 500px; min-height: 80px;  margin-left: auto; margin-right: auto; padding-top: 2%; padding-bottom: 30px }
	...
	/* 헤더끝 */
	/* 컨테이너 시작 */
	#containerAgree{height: 100vh;  margin-left: auto; margin-right: auto; background-color: #F0F0E1}
	#agreefrm{ height: 100% ; width: 100%}
	#agreewrap{
	margin-left: auto; 
	margin-right: auto;
	 height: 250px;
	...
	#agreeC1,#agreeC2{ color:#FF0000; font-weight: bold; display: none; }
</style>
...
$(function(){
	$("#btnJoin").click(function(){
		...
		if(!$("#agree1").is(":checked")){
			for (var i = 0; i < 10; i++) {
			$("#agreeC1").fadeIn(500).fadeOut(500);
			}
			return;
		}
		//end if
		if(!$("#agree2").is(":checked")){
			for (var i = 0; i < 10; i++) {
			$("#agreeC2").fadeIn(500).fadeOut(500);
			}
			return;
		}//end if
		
		location.href="sign_up_frm.do";/*   */
	});//click
...
<div id="header">
	<div id="hContent" >
	  <a id="hTitle" class="navbar-brand" href="index.do" >
	    <img id="hLogo" src="http://localhost:8080/final_prj/common/memberImg/logo.png" class="d-inline-block align-center" alt="" >
	    Yul's Coffee  
	  </a>
	</div>   
</div>
<div id="containerAgree">
	<p class="text-center"> 회원가입약관  <span style="color: #6B94CD">(필수)</span></p>  
	<div id="agreewrap">
	<textarea readonly="readonly" id="agreefrm">
	
	제1장 총 칙

제1조(목적)
이 약관은 "Yul's Coffee"(이하 "회사"라 한다)가 홈페이지에서 제공하는 모든 서비스(이하 "서비스"라 한다)의 이용조건 및 절차에 관한 사항을 규정함을 목적으로 합니다. 
...
(시행일) 이 약관은 2014년 2월 28일부터 시행합니다.
	...
	<div>
	<p class="text-right" >회원가입약관의 내용에 동의합니다. 
	<input type="checkbox" id="agree1" name="agree1">
	<span id="agreeC1" >회원가입 약관에 체크해 주세요</span>
	</p>
	</div>
	<p class="text-center" style="margin-top: 40px"> 개인정보처리방침안내 <span style="color: #6B94CD">(필수)</span></p>  
	<textarea readonly="readonly" id="agreefrm">
	
	개요 및 목록

'Yul's Coffee'는 (이하 '회사'는) 이용자님의 개인정보를 중요시하며,
...
현 개인정보취급방침의 내용이 변경될 경우에는 개정 최소 7일전부터 홈페이지의 "공지사항"을 통해 고지 하겠습니다.
	...
	<span id="agreeC2" class="text-right">개인정보동의 약관에 체크해 주세요</span>
	</p>
	<button type="button" id="btnCancel" class="btn btn-secondary btn-lg" > 취소  </button><!-- 취소하면 메인 페이지로   -->
	<button type="button" id="btnJoin"  class="btn btn-dark btn-lg">확인</button><!-- 확인하면 회원가입이 완료되었습니다. modal 로 띄우기 -->
	</div>
	</div>
</div>
